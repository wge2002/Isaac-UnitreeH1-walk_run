# 这个python文件都和命令行参数相关，设置命令行，或者解析命令行参数。
...
def add_rsl_rl_args(parser: argparse.ArgumentParser):
    """Add RSL-RL arguments to the parser. 将RSL-RL参数添加到解析器中。
...
def parse_rsl_rl_cfg(task_name: str, args_cli: argparse.Namespace) -> RslRlOnPolicyRunnerCfg:
    """Parse configuration for RSL-RL agent based on inputs. 基于输入解析RSL-RL代理的配置。
    ...
    rslrl_cfg: RslRlOnPolicyRunnerCfg = load_cfg_from_registry(task_name, "rsl_rl_cfg_entry_point")  # task判断
